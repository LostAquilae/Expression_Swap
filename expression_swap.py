...
left_swap = Swap(dst_img_init, dst_rect_face_init, src_img_init, src_rect_face_init, shape_predictor)
...
while True:
    ...
    if constants._DEBUG_:
        ls_debug_img = cam_img_gray.copy()
        rs_debug_img = cam_img_gray.copy()
    ...
    if len(faces) > 1:
        ...
        if len(left_img) != 0:
            left_img_saved = left_img.copy()
            left_img = left_swap.getFrame(right_img, right_rect_face, left_img, left_rect_face)
            right_img = right_swap.getFrame(left_img_saved, left_rect_face, right_img, right_rect_face)
            ...
            if constants._DEBUG_:
                ls_right_debug_img, ls_left_debug_img = left_swap.getDebugFrame()
                ls_debug_img[0:ls_debug_img.shape[0], 0:ls_left_debug_img.shape[1]] = ls_left_debug_img
                ls_debug_img[0:ls_debug_img.shape[0], ls_left_debug_img.shape[1]:ls_debug_img.shape[1]] = ls_right_debug_img

                rs_left_debug_img, rs_right_debug_img = right_swap.getDebugFrame()
                rs_debug_img[0:rs_debug_img.shape[0], 0:rs_left_debug_img.shape[1]] = rs_left_debug_img
                rs_debug_img[0:rs_debug_img.shape[0], rs_left_debug_img.shape[1]:rs_debug_img.shape[1]] = rs_right_debug_img
    ...
    if constants._DEBUG_:
        ...
        cv2.putText(rs_debug_img, fps, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255))
        cv2.imshow("RIGHT_SWAP_DEBUG", rs_debug_img)
        cv2.imshow("LEFT_SWAP_DEBUG", ls_debug_img)
