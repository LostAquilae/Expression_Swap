...
if len(sys.argv) != 3:
    print("python expression_swap.py predictor.dat image.jpg")
    exit()
...
# Resize image
dst_scale = dst_img.shape[1] / dst_img.shape[0]
dst_img = cv2.resize(dst_img, (int(_HEIGHT_IMG_ * dst_scale), _HEIGHT_IMG_))

# Converting to gray scale to make optimizations
dst_img_gray = cv2.cvtColor(dst_img, cv2.COLOR_BGR2GRAY)
dst_img_gray_save = dst_img_gray.copy()

# Get dst face
dst_rect_face = getFace(dst_img_gray)
if dst_rect_face == None:
    print("No face found in destination image.")
    exit()
...
while True:
    ...
    if src_rect_face != None:
        ...
        areLandmarksInImages = True
        # Check landmarks in the images
        for i in range(0, len(src_landmarks)):
            if src_landmarks[i][0] < 0 or src_landmarks[i][1] < 0 or src_landmarks[i][0] > dst_img.shape[0] or src_landmarks[i][1] > dst_img.shape[1]:
                areLandmarksInImages = False
                break

        if areLandmarksInImages:
            ...
            for i in range(0, len(src_points)):
                ...
                if _DEBUG_:
                    # Display landmarks
                    cv2.circle(dst_img_gray, dst_landmark_new, 2, (0, 0, 255), 2)
            ...
            dst_img_new = np.zeros_like(dst_img_gray)
    ...
    if _DEBUG_:
        ...
        cv2.putText(src_img_gray, fps, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255))

        if src_rect_face != None:
            # Display rect around face
            cv2.rectangle(src_img_gray, (src_convex_rect[0], src_convex_rect[1]),
                        (src_convex_rect[0] + src_convex_rect[2], src_convex_rect[1] + src_convex_rect[3]), (255, 255, 255), 2)

        cv2.imshow("DEBUG_Source", src_img_gray)
        cv2.imshow("DEBUG_Destination", dst_img_gray)
        cv2.imshow("DEBUG_Final_Destination", img_new)
    else:
        cv2.imshow("Source", src_img)
        cv2.imshow("Destination", img_new)
